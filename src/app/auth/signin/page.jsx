...
const Signin = () => {
    ...
    if (validate_signup()) {
      const fetchData = async () => {
        ...
        try {
          ...
          response?.memberCheckLogin?.email
            ? response?.memberCheckLogin?.password
              ? (setPasswordError(""), setPasswordStateError(false))
              : (setPasswordError("Invalid Password"),
                setPasswordStateError(true))
            : (setEmailError("Invalid Email"), setEmailStateError(true));
          ...
          localStorage.setItem(
            "Id",
            response?.memberCheckLogin?.memberDetails?.Id
          );
          localStorage.setItem(
            "NameString",
            response?.memberCheckLogin?.memberDetails?.NameString
          );
  ...
  // useEffect(() => {
  //   dispatch(get_user_id_Redux_func(id));
  //   console.log(id, "bsixj");
  // }, [nameInitial, id]);
      ...
      <section className="bg-[#FAFAFA] min-h-[calc(100vh-120px)] p-[26px_16px] flex flex-col max-md:min-h-[calc(100vh-68px)] max-[1300px]:p-[16px] max-[1300px]:min-h-[calc(100vh-79px)]">
        ...
        <div className="w-[90%] mx-auto max-[1400px]:w-full  mb-auto">
          <div className="max-w-[394px] mx-auto">
            <h1 className="text-[36px] font-semibold leading-[43px] text-[#1D1D1F] text-center mb-[16px] max-sm:text-[28px] max-[1300px]:mb-[10px]">
              Log in to your account
            </h1>
            <p className="text-base font-medium leading-[17px] text-[#83838D] text-center mb-[24px] max-[1300px]:mb-[16px]">
            ...
            <div className="bg-[#FFFFFF] border border-[#E9E9E9] p-[30px] rounded-[12px]">
              ...
              <div className="mb-[10px] last-of-type:mb-0 relative">
                ...
                {passwordStateError && (
                  <div className=" flex items-start space-x-[4px] mt-[5px]">
                    <img src="/img/error.svg" alt="error" />{" "}
                    <p className="text-[10px] font-normal leading-[12px] text-[#EC1919]">
                      {passwordError}{" "}
                    </p>
              ...
              <button
                onClick={(e) => submit_signup(e)}
                className="bg-[#1D1D1F] border border-[#D8D8D8] text-[14px] leading-[16px] p-[12px] w-full block h-[42px] font-semibold text-[#FFFFFF] mt-[32px] rounded-[4px] text-center hover:bg-[#28282c] max-[1300px]:mt-[16px]"
              >
                Sign In
              </button>
            </div>
